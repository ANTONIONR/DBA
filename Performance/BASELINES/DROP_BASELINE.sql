...
--  ----------------------------------------------------------------------------------------------------
--
--  FILE NAME      : DROP_BASELINE.SQL
--  DESCRIPTION    : This script drop a baseline by plan name
--  AUTHOR         : Antonio NAVARRO - /\/\/
--  CREATE         : 16.10.14
--  LAST MODIFIED  : 16.10.14
--  USAGE          : This script inputs two parameters. Parameter 1 is a plan name
--  CALL SYNTAXIS  : @drop_baseline.sql  my_plan_name
--  [NOTES]        :
--   
--  ----------------------------------------------------------------------------------------------------
...
BEGIN
 ...
 -- Get the handle for sql with planname pass as parameter
  ...
  exit_return := DBMS_SPM.DROP_SQL_PLAN_BASELINE (
   sql_handle     => SqlHandle, 
   plan_name      => :PlanName );

  IF exit_return != -1 THEN 
     DBMS_OUTPUT.PUT_LINE (' BASELINE DROPPED (Plan NAME) : ' || :PlanName);
  ELSE
     DBMS_OUTPUT.PUT_LINE (' ERROR DROPPING BASELINE ');
     RETURN;
  END IF;
