--  ----------------------------------------------------------------------------------------------------
--
--  FILE NAME      : UNFIX_BASELINE.SQL
--  DESCRIPTION    : This script unfixed a baseline from a plan name created
--  AUTHOR         : Antonio NAVARRO - /\/\/
--  CREATE         : 16.10.14
--  LAST MODIFIED  : 16.10.14
--  USAGE          : This script unfixed a baseline from a plan name created
--  CALL SYNTAXIS  : @unfix_baseline.sql  my_plna_name
--  [NOTES]        :
--   
--  ----------------------------------------------------------------------------------------------------
...
BEGIN
  ...
  IF exit_return != -1 THEN 
     DBMS_OUTPUT.PUT_LINE (' BASELINE UNFIXED (Plan NAME) : ' || :PlanName);
  ELSE
     DBMS_OUTPUT.PUT_LINE (' ERROR UNFIXING BASELINE ');
     RETURN;
  END IF;
